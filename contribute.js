...
document.querySelectorAll("[data-method]").forEach((button)=>button.addEventListener("click",()=>{setMethod(button.dataset.method);if(button.dataset.method==="upload")document.querySelector("[data-upload-file]").click();}));
...
async function readUpload(file) {
  ...
  if (!file.name.toLowerCase().endsWith(".json")) { setStatus(status,"Choose a .json Fydor pack. Other file types are not accepted."); return; }
...
function clearUpload() {
  ...
  setStatus(document.querySelector("[data-upload-status]"),"Choose another .json pack.");
