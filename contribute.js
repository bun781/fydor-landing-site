...
let validatedPack=null; let draft=null; let sentenceIndex=0; const reviews=new Map(); let prompt=""; let generationSource="manual"; let creationMethod="ai";
...
function setPhase(next){phase=next;for(const section of stepSections)section.hidden=Number(section.dataset.step)!==phase;for(const indicator of stepper.querySelectorAll("[data-step-indicator]")){const step=Number(indicator.dataset.stepIndicator);indicator.classList.toggle("active",step===phase);indicator.classList.toggle("complete",step<phase);if(step===phase)indicator.setAttribute("aria-current","step");else indicator.removeAttribute("aria-current");}const workspaceStatus=document.querySelector("[data-import-status]");workspaceStatus.hidden=phase<2||phase>3||creationMethod==="upload";if(creationMethod==="upload")uploadPanel.hidden=phase!==3;}

setupAuth(async()=>{contributorEntry.hidden=true;workspace.hidden=false;stepper.hidden=false;submissionsSection.hidden=false;setPhase(conversionSource?3:1);await Promise.all([loadDrafts(),loadSubmissions()]);if(conversionSource)setStatus(importStatus,"A personal lesson was copied from Fydor. Paste it into the lesson JSON field, then continue through the same contributor review workflow.");});
