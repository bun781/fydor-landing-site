...
export type SubmissionDetail = Submission & { source_draft_id: string };
...
export function SubmissionStatus({ submission, versions, feedback, onRevise, onBack }: { submission: SubmissionDetail; versions: SubmissionVersion[]; feedback: Feedback[]; onRevise: (draftId: string) => void; onBack: () => void }) {
  ...
  return <div className="stack">
    <section className="workspace-card compact-card"><button className="text-button" onClick={onBack}>← My packs</button><div className="workspace-header"><div><span className="eyebrow">Moderation status</span><h1>{submission.title}</h1><p>{submission.target_language} → {submission.base_language} · Submission version {submission.current_version}</p></div><span className={`submission-state state-${submission.state}`}>{submission.state.replaceAll("_", " ")}</span></div>
      ...
      {canRevise ? <div className="callout"><div><strong>A revised submission is available.</strong><p>Open the preserved draft, address feedback, and approve the latest sentence revisions before resubmitting.</p></div><button className="button" onClick={() => onRevise(submission.source_draft_id)}>Revise draft</button></div> : null}
    </section>
