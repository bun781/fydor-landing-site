...
function configuredWebOrigin(env = process.env) {
  ...
  const configured = env.FYDOR_WEB_ORIGIN || env.NEXT_PUBLIC_FYDOR_WEB_ORIGIN || env.VITE_FYDOR_WEB_ORIGIN || previewOrigin || DEFAULT_PRODUCTION_ORIGIN;
  const productionOrigin = env.VERCEL_ENV === "production" && isLocalOrigin(configured)
    ? DEFAULT_PRODUCTION_ORIGIN
    : configured;
  return normalizeWebOrigin(productionOrigin);
}
