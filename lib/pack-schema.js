...
const PACK_KEYS = ["type", "schemaVersion", "id", "title", "description", "author", "version", "license", "language", "baseLanguage", "level", "tags", "createdAt", "updatedAt", "unitManifest", "grammarGuide", "lessons"];
...
const GRAMMAR_KEYS = ["pattern", "surface", "meaning", "explanation", "ruleId"];
const CHUNK_KEYS = ["surface", "meaning", "explanation", "type", "level", "tags"];

function parseAndValidatePack(source, options = {}) {
  ...
  scanValue(value, 0);
  const issues = validatePack(value, options);
  if (issues.length) {
...
function parseAndValidateDraftPack(source) {
  return parseAndValidatePack(source, { allowIncomplete: true });
}

function validatePack(value, options = {}) {
  ...
  validateText(value.title, "title", issues, { required: !allowIncomplete, max: 200 });
  ...
  if (!allowIncomplete && !value.lessons.length) issues.push(issue("lessons", "must contain at least one lesson"));
  ...
  value.lessons.forEach((lesson, index) => {
    validateLesson(lesson, `lessons[${index}]`, issues, options);
  ...
  validatePackAnnexes(value, issues);
  return issues;
}

function validatePackAnnexes(pack, issues) {
  if (pack.unitManifest !== undefined) {
    if (!isRecord(pack.unitManifest)) issues.push(issue("unitManifest", "must be an object"));
    else {
      validateRecord(pack.unitManifest, "unitManifest", ["schemaVersion", "units"], issues);
      if (pack.unitManifest.schemaVersion !== 1) issues.push(issue("unitManifest.schemaVersion", "must equal 1"));
      if (!Array.isArray(pack.unitManifest.units) || !pack.unitManifest.units.length) issues.push(issue("unitManifest.units", "must contain at least one unit"));
      else {
        const ids = new Set(); const positions = new Set(); const assigned = new Set();
        pack.unitManifest.units.forEach((unit, index) => {
          const path = `unitManifest.units[${index}]`;
          if (!isRecord(unit)) { issues.push(issue(path, "must be an object")); return; }
          validateRecord(unit, path, ["id", "title", "position", "lessonIndexes"], issues);
          validateText(unit.id, `${path}.id`, issues, { required: true, max: 160 });
          validateText(unit.title, `${path}.title`, issues, { required: true, max: 200 });
          if (!Number.isInteger(unit.position) || unit.position < 0) issues.push(issue(`${path}.position`, "must be a non-negative integer"));
          if (!Array.isArray(unit.lessonIndexes) || !unit.lessonIndexes.length) issues.push(issue(`${path}.lessonIndexes`, "must contain at least one lesson index"));
          if (ids.has(unit.id)) issues.push(issue(`${path}.id`, "duplicates another unit id")); ids.add(unit.id);
          if (positions.has(unit.position)) issues.push(issue(`${path}.position`, "duplicates another unit position")); positions.add(unit.position);
          for (const lessonIndex of unit.lessonIndexes || []) {
            if (!Number.isInteger(lessonIndex) || lessonIndex < 0 || lessonIndex >= pack.lessons.length) issues.push(issue(`${path}.lessonIndexes`, "references a missing lesson"));
            if (assigned.has(lessonIndex)) issues.push(issue(`${path}.lessonIndexes`, "assigns a lesson more than once"));
            assigned.add(lessonIndex);
          }
        });
        if (assigned.size !== pack.lessons.length) issues.push(issue("unitManifest", "must assign every lesson exactly once"));
      }
    }
  }
  if (pack.grammarGuide !== undefined) {
    if (!isRecord(pack.grammarGuide)) issues.push(issue("grammarGuide", "must be an object"));
    else {
      validateRecord(pack.grammarGuide, "grammarGuide", ["schemaVersion", "rules"], issues);
      if (pack.grammarGuide.schemaVersion !== 1) issues.push(issue("grammarGuide.schemaVersion", "must equal 1"));
      if (!Array.isArray(pack.grammarGuide.rules) || !pack.grammarGuide.rules.length) issues.push(issue("grammarGuide.rules", "must contain at least one rule"));
      else {
        const ids = new Set(); const referenced = new Set();
        pack.grammarGuide.rules.forEach((rule, index) => {
          const path = `grammarGuide.rules[${index}]`;
          if (!isRecord(rule)) { issues.push(issue(path, "must be an object")); return; }
          validateRecord(rule, path, ["id", "title", "overview", "situations", "contrasts", "commonMistakes"], issues);
          validateText(rule.id, `${path}.id`, issues, { required: true, max: 160 });
          validateText(rule.title, `${path}.title`, issues, { required: true, max: 500 });
          validateText(rule.overview, `${path}.overview`, issues, { required: true, max: 4_000 });
          if (ids.has(rule.id)) issues.push(issue(`${path}.id`, "duplicates another grammar rule id")); ids.add(rule.id);
          validateStringArray(rule.contrasts, `${path}.contrasts`, issues, 30, 4_000);
          validateStringArray(rule.commonMistakes, `${path}.commonMistakes`, issues, 30, 4_000);
          if (!Array.isArray(rule.situations) || !rule.situations.length) issues.push(issue(`${path}.situations`, "must contain at least one situation"));
          else rule.situations.forEach((situation, situationIndex) => {
            const situationPath = `${path}.situations[${situationIndex}]`;
            if (!isRecord(situation)) { issues.push(issue(situationPath, "must be an object")); return; }
            validateRecord(situation, situationPath, ["title", "explanation", "unitIndexes"], issues);
            validateText(situation.title, `${situationPath}.title`, issues, { required: true, max: 500 });
            validateText(situation.explanation, `${situationPath}.explanation`, issues, { required: true, max: 4_000 });
            if (!Array.isArray(situation.unitIndexes) || !situation.unitIndexes.length) issues.push(issue(`${situationPath}.unitIndexes`, "must contain at least one unit index"));
          });
        });
        pack.lessons.forEach((lesson) => lesson.sentences.forEach((sentence) => sentence.grammar?.forEach((grammar) => { if (grammar.ruleId) referenced.add(grammar.ruleId); })));
        for (const rule of pack.grammarGuide.rules) if (!referenced.has(rule.id)) issues.push(issue("grammarGuide.rules", `rule ${rule.id} has no sentence examples`));
        for (const ruleId of referenced) if (!ids.has(ruleId)) issues.push(issue("lessons.grammar.ruleId", `references unknown grammar rule ${ruleId}`));
      }
    }
  }
}

function validateLesson(value, path, issues, options = {}) {
  const allowIncomplete = options.allowIncomplete === true;
  ...
  validateText(value.title, `${path}.title`, issues, { required: !allowIncomplete, max: 200 });
  ...
  if (!allowIncomplete && !value.sentences.length) issues.push(issue(`${path}.sentences`, "must contain at least one sentence"));
  ...
  value.sentences.forEach((sentence, index) => {
    ...
    validateSentence(sentence, sentencePath, issues, options);
    if (typeof sentence?.text === "string") {
      const key = sentence.text.normalize("NFC").trim().toLocaleLowerCase();
      if (!allowIncomplete && seen.has(key)) issues.push(issue(`${sentencePath}.text`, "duplicates another sentence"));
      seen.add(key);
    }
  });
}

function validateSentence(value, path, issues, options = {}) {
  const allowIncomplete = options.allowIncomplete === true;
  if (!isRecord(value)) { issues.push(issue(path, "must be an object")); return; }
  validateRecord(value, path, SENTENCE_KEYS, issues);
  validateText(value.text, `${path}.text`, issues, { required: !allowIncomplete, max: MAX_STRING });
  validateText(value.translation, `${path}.translation`, issues, { required: !allowIncomplete, max: MAX_STRING });
  validateAnnotations(value.words, `${path}.words`, "surface", WORD_KEYS, value.text, issues, options);
  validateAnnotations(value.grammar, `${path}.grammar`, "pattern", GRAMMAR_KEYS, value.text, issues, options);
  validateAnnotations(value.chunks, `${path}.chunks`, "surface", CHUNK_KEYS, value.text, issues, options);
}

function validateAnnotations(items, path, requiredField, allowedKeys, sentenceText, issues, options = {}) {
  const allowIncomplete = options.allowIncomplete === true;
  ...
  items.forEach((item, index) => {
    ...
    validateText(item[requiredField], `${itemPath}.${requiredField}`, issues, { required: !allowIncomplete, max: 500 });
    ...
    // Grammar patterns describe a rule and are not necessarily literal text.
    // Only an explicitly supplied surface annotation must be present verbatim.
    const surface = item.surface;
    if (!allowIncomplete && typeof surface === "string" && typeof sentenceText === "string" && !sentenceText.normalize("NFC").includes(surface.normalize("NFC"))) {
      issues.push(issue(`${itemPath}.surface`, "must occur in the sentence text"));
    }
    const key = stableStringify(item);
    if (!allowIncomplete && seen.has(key)) issues.push(issue(itemPath, "duplicates another annotation"));
    seen.add(key);
  });
}
...
module.exports = {
  ...
  parseAndValidateDraftPack, parseAndValidatePack, slugifyPackTitle, stableStringify
};
