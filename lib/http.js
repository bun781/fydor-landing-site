...
function setCors(request, response, options = {}) {
  ...
  else if (origin && (origin === configured || origin === preview)) {
...
function requireSameOrigin(request) {
  ...
  const configured = configuredWebOrigin();
  const preview = process.env.VERCEL_ENV === "preview" && process.env.VERCEL_URL ? `https://${process.env.VERCEL_URL}` : null;
  if (origin && origin !== configured && origin !== preview) throw httpError(403, "cross_origin_request", "Cross-origin requests are not allowed.");
  const fetchSite = String(request.headers["sec-fetch-site"] || "");
  if (fetchSite === "cross-site") throw httpError(403, "cross_origin_request", "Cross-origin requests are not allowed.");
}
...
module.exports = { handleOptions, httpError, isTrustedDesktopOrigin, readJsonBody, requireMethod, requireSameOrigin, safeServerError, sanitizeJsonValue, send, sendError, setCors };
