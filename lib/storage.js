...
async function deletePackObject(path, options = {}) {
  const config = storageConfig(options.env);
  const bucket = normalizeBucket(options.bucket || process.env.SUPABASE_PACK_BUCKET || DEFAULT_PACK_BUCKET);
  const response = await fetch(storageObjectUrl(config.supabase, bucket, path), {
    method: "DELETE",
    headers: serviceAuthHeaders(config.supabaseSecretKey)
  });
  if (response.status === 404) return false;
  if (!response.ok) {
    const text = await response.text();
    const message = String(text || `Storage request failed (${response.status}).`).replace(/service[_ -]?role|jwt|token|key|bearer/gi, "credential");
    throw httpError(response.status === 404 ? 404 : 400, "storage_rejected", message);
  }
  return true;
}
...
module.exports = { DEFAULT_PACK_BUCKET, deletePackObject, listPackObjects, readPackObject, storageObjectUrl, storagePublicUrl, uploadPackObject };
