...
const PROMPT_TEMPLATE_VERSION = "fydor-pack-v1.0.0";
...
function buildLessonPrompt(input) {
  ...
  return {
    ...
    prompt: `You are generating an unreviewed Fydor lesson pack. Return exactly one strict JSON object, with no Markdown fence and no commentary.
...
Before returning, check that the JSON parses, uses type fydor_pack and schemaVersion ${schemaVersion}, contains only the allowed fields, and satisfies every surface rule.`
  };
}
